...
/*
This software is the property of Applied Research 
Associates, Inc.,  4300 San Mateo Blvd NE,
Suite A-220, Albuquerque, NM 87110

(c) 2007-2009 Applied Research Associates, Inc.
All Rights Reserved.  Unpublished rights 
reserved under the Copyright Laws of
the United States.

U.S. GOVERNMENT RIGHTS
Use, duplication, or diclosure by the U.S. Government
is subject to restirctions set forth in the 
Applied Research Associates, Inc. license
agreement and as provided in DFARS 227.7202-1(a)
and 227.7202-3a (1995), DFARS 252.227-7013(c)(1)(ii)
(Oct 1988), FAR 12.212(a) (1995), FAR 52.227-19 or
FAR 52.227-14(ALT III), as applicable.
Applied Research Associates, Inc.

THIS SOFTWARE CONTAINS CONFIDENTIAL
INFORMATION AND TRADE SECRETS OF 
APPLIED RESEARCH ASSOCIATES, INC.
USE, DISCLOSURE, OR REPRODUCTION IS
PROHIBITED WITHOUT THE PRIOR EXPRESS
WRITTEN PERMISSION OF APPLIED RESEARCH
ASSOCIATES, INC.
*/
#include "stdafx.h"
#include "math.h"
...
/* AC joint reflection cracking */
#ifdef ACCURATE_BUT_PROBABLY_NOT_IN_MICROPAVER
...
/* M = 253*/ { {0, 0}, {5, 4.5f}, {10, 9}, {20, 17.f}, {30, 24}, {40, 28}, {50, 31.5f}, {60, 33}, {70, 34.5f}, {80, 36}, {90, 37.5f}, {100, 39.5f}, {1100, 45} },     /* */
...
/*
 * dis2row -- given the SAL distress number from [0-SALDISTRESSRANGE]
 *	      and the severity number (0=high, 1=medium, 2=low)
 *	      dis2row returns the index of the row of interpolation
 *	      data in the interpolation table to use for this
...
/***************************************************************************/
/*
 * Q(int i, float x) -- this is nothing more than an
 *   indexed library of functions f(a): [0,1000]->R+.
 *
 *   Semantically Q(i,x) is the a'th q-curve evaluated at pt x.
 */
 
float Distress::Q(int i, float x)
{
static struct { short x,y; } d[NUMQ][NUMPTS] = {
...
//
// Description: Returns the deduct value for a non PCI distress
//
...
//
// Description: Returns the deduct value for a single distress. 
// Ok to pass in '-' 'nun' or whatever for sev or extent level, 
// for distresses without defined levels.
//
...
// pvt_SeverityExtent2Deduct('M', 'M', 2, 3, 4, 32, 33, 34, 88, 89, 90) returns 3
// A 4th value severity or extent is silently mapped to an un-specified one of the other levels
// For those distresses call the function with replicated parameters
//				
//
