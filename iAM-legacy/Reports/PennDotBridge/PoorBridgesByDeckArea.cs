...
namespace Reports.PennDotBridge
{
    ...
    {
        public PoorBridgesByDeckArea(string networkId, string simulationId, _Worksheet oSheet,BridgeAnalysis analysis)
        ...
        {
            const string title = "Poor Bridges by Deck Area";
            Report.SheetPageSetup(Sheet, title, 50d, 20d, 10d, "", DateTime.Now.ToLongDateString(), "Page &P", 6);
            WriteHeader(title);
        ...
        private void WriteAnalysisData()
        {
            ...
            var network1 = Analysis.GetSdDeckArea("1");
            var network2 = Analysis.GetSdDeckArea("2");
            var network3 = Analysis.GetSdDeckArea("3");
            var network4 = Analysis.GetSdDeckArea("4");
