...
namespace Reports
{
    public class BudgetPerConditionReport
    {
        string m_strNetworkID, m_strSimulationID, m_strNetwork, m_strSimulation, m_strBudgetPer;

        public BudgetPerConditionReport(string strNetworkID, string strSimulationID, string strNetwork, string strSimulation, string strBudgetPer)
        {
        ...
        public void CreateBudgetPerConditionReport()
        {
            ...
            string strReportName = "Budget Per Condition Report";
			...
			switch (DBMgr.NativeConnectionParameters.Provider)
			{
				...
				case "ORACLE":
					...
					break;
				default:
					throw new NotImplementedException("TODO: Create ANSI implementation for CreateBudgetPerConditionReport()");
					//break;
			}
            ...
            Hashtable budPerCondition;
            double nTmp, nTmp1; //, nAcceptible, nUnacceptible;
            ndx = 0;
            oData[ndx, 0] = "Budget Distribution ($) per Condition Category"; // Group Header
            ...
            foreach (string s in listCondition)
            {
                ...
                foreach (DataRow dr in dsBudgetYears.Tables[0].Rows)
                {
                    //budPerCondition = DBOp.GetBudgetTotals(m_strNetworkID, m_strSimulationID, dr["Year"].ToString(), strBudget, m_strBudgetPer, strFilter);
                    //strFilter = m_strBudgetPer + " = '" + s.ToUpper() + "'" + " AND Budget IS NOT NULL";
                    strFilter = m_strBudgetPer + "_" + dr["Year_"].ToString() + " = '" + s.ToUpper() + "'" + " AND Budget IS NOT NULL";
                    budPerCondition = DBOp.GetBudgetTotals(m_strNetworkID, m_strSimulationID, dr["Year_"].ToString(), "", "", strFilter);
                    string strHash = (string)budPerCondition["All"];
            ...
            oData[ndx, 0] = "Budget Distribution (%) per Condition Category";
            ndx++;
            sheetRow++;
            ...
            Report.CreateColClusterBarGraph(left, 15, 425, 315, oSheet, oSourceData, "='Budget Per Condition Report'!$B$5:$" + Report.GetColumnLetter(aryCols) + "$5",
                "Annual Budget Per Condition", 12, "Fiscal Year", 11, listCondition, "", 11, XlRowCol.xlRows);
            #endregion
