...
{
	public class ConditionCitywidePercent
	{
		private int m_networkID;
		private string m_userName;
		//private Dictionary<string, double> m_PCIBins;

		public ConditionCitywidePercent(int networkID, string userName)
		{
		...
		public void CreateConditionCitywidePercentReport()
		{
			...
			binnedPCIData = PCIBins.CreateAttributeBins(binAttributes, Method.PERCENTAGE);
		...
		private void GenerateReport(Dictionary<string, double> binnedPCIData)
		{
			...
			ReportGlobals.CreateReportHeader("Condition Citywide (%)", "", Environment.GetFolderPath(Environment.SpecialFolder.MyDocuments) + @"\RoadCare3 Documents\Washington DC\ddot logo.gif", m_userName);
			...
			pciCitywide.ChartTitle.Text = "City PCI as Percentage of Total Area";
