...
{
	public class MileageByQuadrant
	{
		string userName;
		int networkID;

		public MileageByQuadrant(int netID, string usrName )
		{
			networkID = netID;
			userName = usrName;
		}
		...
		{
			string query = "SELECT QUADRANT, SUM(QUAD_LENGTH) / 5280 AS QUAD_LENGTH FROM (SELECT QUADRANT.DATA_ AS QUADRANT, MAX(LENGTH.DATA_) AS QUAD_LENGTH FROM LENGTH INNER JOIN QUADRANT ON QUADRANT.FACILITY = LENGTH.FACILITY AND QUADRANT.SECTION = LENGTH.SECTION GROUP BY QUADRANT.DATA_, LENGTH.FACILITY, LENGTH.SECTION) AS DERIVEDTABLE GROUP BY QUADRANT";
			DataSet quadData = DBMgr.ExecuteQuery(query);


			SetUpWorkBook("MileageByQuadrant");
			CreateReportHeader("Mileage By Quadrant");
			CreateReportRows(quadData.Tables[0]);
			SaveReport();
			ReportGlobals.XLMgr.CloseWorkbook("MileageByQuadrant");
		}
