...
namespace Jace.Tokenizer
{
    ...
    public class TokenReader
    {
        ...
        public List<Token> Read(IEnumerable<char> formula)
        {
            var tokens = new List<Token>();

            if (formula == null) return tokens;

            var characters = formula.ToArray();
        ...
        {
            return (character >= 'a' && character <= 'z') || (character >= 'A' && character <= 'Z') || (!isFirstCharacter && character >= '0' && character <= '9') || character == '_'; // ARA
        }
