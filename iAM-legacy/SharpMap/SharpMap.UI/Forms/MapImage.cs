...
namespace SharpMap.Forms
{
    ...
    public class MapImage : System.Windows.Forms.PictureBox
    {
        ...
        /// </summary>
        /// <param name="data"></param>
        public delegate void MapQueryHandler(DataTable data);
        /// <summary>
                    ...
                    else if (this.ActiveTool == Tools.Query)
                    {
                        if (_Map.Layers.Count > _queryLayerIndex && _queryLayerIndex > -1)
                        {
                            if (_Map.Layers[_queryLayerIndex].GetType() == typeof(SharpMap.Layers.VectorLayer))
                            {
                                ...
                                //String strLayerName = layer.LayerName;
                                //String strSelect = "SELECT * FROM " + strLayerName + " WHERE ";
                                //strSelect += bbox.Left.ToString() + " < LONGITUDE AND LONGITUDE > " + bbox.Right.ToString() + " AND LATITUDE < " + bbox.Top.ToString() + " AND LATITUDE > " + bbox.Bottom.ToString();
                                //DataSet ds = DBMgr.ExecuteQuery(strSelect);

                                //layer.DataSource.Open();
                                //layer.DataSource.ExecuteIntersectionQuery(layer.ToString(),bbox, ds);
