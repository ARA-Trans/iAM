...
namespace SharpMap.Data.Providers
{
	...
	public class MsSql : IProvider, IDisposable
	{
					...
					using (SqlDataReader dr = command.ExecuteReader())
					{
						while (dr.Read())
						{
							if (dr[0] != DBNull.Value)
							{
                                ...
                                SharpMap.Geometries.Geometry geom = SharpMap.Converters.WellKnownText.GeometryFromWKT.Parse(dr[0].ToString());
								if (geom != null)
									features.Add(geom);
							}
						}
        ...
        ///// <param name="ds">FeatureDataSet to fill data into</param>
        //public void ExecuteIntersectionQuery(String strLayerName, SharpMap.Geometries.BoundingBox box, FeatureDataSet ds)
        //{
        //    throw new NotImplementedException();
        //}
			...
			using (SqlConnection conn = new SqlConnection(_ConnectionString))
			{
				...
				using (SqlCommand command = new SqlCommand(strSQL, conn))
				{
					...
					using (SqlDataReader dr = command.ExecuteReader())
						if (dr.Read())
						{
                            box = new SharpMap.Geometries.BoundingBox(float.Parse(dr[0].ToString()), float.Parse(dr[1].ToString()), float.Parse(dr[2].ToString()), float.Parse(dr[3].ToString()));
						}
					conn.Close();
				}
				return box;
