...
namespace SharpMap.Rendering
{
	...
	public class VectorRenderer
	{
		...
		private static Dictionary<Color, Bitmap> m_dictionaryColorBitmap = new Dictionary<Color, Bitmap>();
		/// <summary>
		...
		private static Bitmap ConvertBitmapColor(Color colorOld, Color colorNew, Bitmap bmp)
		{
			if (m_dictionaryColorBitmap.ContainsKey(colorNew))
			{
				return m_dictionaryColorBitmap[colorNew];
			}
			else
			{
				Bitmap transformedBitmap = new Bitmap(bmp);
				for (int i = 0; i < transformedBitmap.Width; i++)
				{
					for (int j = 0; j < transformedBitmap.Height; j++)
					{
						Color cTemp = transformedBitmap.GetPixel(i, j);
						if (cTemp.R == colorOld.R && cTemp.B == colorOld.B && cTemp.G == colorOld.G)
						{
							transformedBitmap.SetPixel(i, j, colorNew);
						}
					}
				}
				m_dictionaryColorBitmap.Add(colorNew, transformedBitmap);
